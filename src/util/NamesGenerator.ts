/*
Adapted from docker names generator https://github.com/moby/moby/blob/master/pkg/namesgenerator/names-generator.go
 */
...
export default class NamesGenerator {
     ...
     */
    public static getRandomName(retry = 0): string {
        const left = NamesGenerator.left;
        const right = NamesGenerator.right;
        let name;

        do {
            name = `${left[Math.round(Math.random() * left.length)]}_${right[Math.round(Math.random() * right.length)]}`;
